...
public class AuthController : MainController
{
    ...
    public async Task<ActionResult> Registrar(RegisterUserViewModel registerUser)
    {
        ...
        if (result.Succeeded)
        {
            ...
            return CustonResponse( await GerarJwt(user.Email));
        }
    ...
    public async Task<ActionResult> Login(LoginUserViewModel loginUser)
    {
        ...
        if (result.Succeeded)
        {
            return CustonResponse( await GerarJwt(loginUser.Email));
        }

        if (result.IsLockedOut)
        {
            NotificarErro("Usuário termporariamente bloqueado por tentativas inválidas");
        }
    ...
    private async Task<LoginResponseViewModel> GerarJwt(string email)
    {
        ...
        var token = tokenHandler.CreateToken(new SecurityTokenDescriptor
        {
            ...
            SigningCredentials = new SigningCredentials(new SymetricSecurity(key), SecurityAlgorithms.HmacSha256)
        });
